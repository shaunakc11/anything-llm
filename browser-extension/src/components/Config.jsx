...
export default function Config({ status, onStatusChange }) {
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!online) {
        setSaveStatus(
          "OssorioIA is currently offline. Please try again later."
        );
        return;
      }
      ...
      if (response.ok) {
        ...
        onStatusChange();
        setSaveStatus("Successfully connected to OssorioIA");
  ...
  const handleDisconnect = async () => {
    try {
      ...
      onStatusChange();
      setSaveStatus("Successfully disconnected from OssorioIA");
      chrome.runtime.sendMessage({ action: "connectionUpdated" });
    } catch (error) {
  ...
  return (
    <div className="w-full flex flex-col gap-y-4">
      ...
      {saveStatus && (
        <div
          className={`p-2.5 rounded-lg ${saveStatus.includes("Successfully")
