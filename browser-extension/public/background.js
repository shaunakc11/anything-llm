  ...
  async create(workspaces) {
    ...
    if (workspaces && workspaces.length > 0) {
      chrome.contextMenus.create({
        id: "saveToOssorioIA",
        title: "Save selected to OssorioIA",
        contexts: ["selection"],
      });
      ...
      chrome.contextMenus.create({
        id: "saveEntirePageToOssorioIA",
        title: "Save entire page to OssorioIA",
        contexts: ["page"],
      });
...
const ExtensionModel = {
  ...
  async saveToOssorioIA(selectedText, pageTitle, pageUrl) {
    const { apiBase, apiKey } = await chrome.storage.sync.get([
      "apiBase",
      "apiKey",
    ]);
  ...
  async saveEntirePageToOssorioIA(pageContent, pageTitle, pageUrl) {
    const { apiBase, apiKey } = await chrome.storage.sync.get([
      "apiBase",
      "apiKey",
    ]);
  ...
  async handleResponse(response, action) {
    if (response.status === 401 || response.status === 403) {
      ...
      this.showNotification(
        "success",
        "Successfully saved content to OssorioIA."
      );
    }
  },
  ...
  showNotification(type, message) {
    ...
    setTimeout(() => {
      ...
      chrome.action.setTitle({ title: "OssorioIA Extension" });
    }, 5000);
  },
};
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "saveToOssorioIA") {
    ExtensionModel.saveToOssorioIA(info.selectionText, tab.title, tab.url);
    return;
  }
  ...
  if (info.menuItemId === "saveEntirePageToOssorioIA") {
    getPageContent(tab.id)
      .then((content) => {
        ExtensionModel.saveEntirePageToOssorioIA(content, tab.title, tab.url);
      })
      .catch((error) => {
