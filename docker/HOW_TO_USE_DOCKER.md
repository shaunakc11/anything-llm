# How to use Dockerized Ossorioia LLM

Use the Dockerized version of OssorioiaLLM for a much faster and complete startup of OssorioiaLLM.

### Minimum Requirements

> [!TIP]
> Running OssorioiaLLM on AWS/GCP/Azure?
...
\*OssorioiaLLM by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*OssorioiaLLM by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized OssorioiaLLM!
...
> the docker container using OssorioiaLLM as `localhost:xxxx` will not resolve for the host system.
>
...
<table>
<tr>
<th colspan="2">Mount the storage locally and run OssorioiaLLM in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
Go to `http://localhost:3001` and you are now using OssorioiagLLM! All your data and progress will persist between
...
- `touch server/storage/ossorioiallm.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of OssorioiaLLM easier.

### Use the Midori AI Subsystem to Manage OssorioiaLLM

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the OssorioiaLLM docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in OssorioiaLLM.
